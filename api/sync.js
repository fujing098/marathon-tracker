...
async function kimiSearch(prompt) {
  try {
    ...
    console.error("Kimi 请求失败：", err.message);
    return [];
  }
}
...
严格返回JSON数组，不要任何其他文字：
[{"title":"资讯标题","source":"媒体来源","date":"YYYY-MM-DD","url":"链接或空字符串","summary":"50字摘要","category":"报名信息或赛事动态或成绩结果","raceType":"马拉松或越野赛或其他赛事","raceInfo":{"name":"赛事名称","city":"城市","raceDate":"YYYY-MM-DD","regStart":"YYYY-MM-DD","regEnd":"YYYY-MM-DD","scale":"规模数字","official":"官网"}}]

raceInfo 只在包含具体报名信息时填写，否则设为 null。`;
...
async function searchWechatHistory(accounts) {
  ...
  const prompt = `请联网搜索以下微信公众号从2026年1月1日到${today}发布的所有内容：${accountList}。只返回与跑步赛事直接相关的内容。严格返回JSON数组，不要任何其他文字：[{"title":"文章标题","source":"公众号名称","date":"YYYY-MM-DD","url":"链接或空字符串","summary":"50字摘要","category":"报名信息或赛事动态或成绩结果","raceType":"马拉松或越野赛或其他赛事","raceInfo":null}]没有相关内容返回 []。`;
  return await kimiSearch(prompt);
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (paramYear && paramMonth) {
      console.log(`搜索 ${paramYear}年${paramMonth}月...`);
